...
async def reenvio_comprobante(
    ...
    try:
        ...
        final_pdf = comp.pdf_path or att["pdf_path"]
        ...
        if all_pdfs and not final_pdf:
            final_pdf = all_pdfs[0]
        elif final_pdf and final_pdf not in all_pdfs:
            all_pdfs.append(os.path.abspath(final_pdf))

        # 4. Fallback Único Generación (Mutuamente excluyente)
        # SUPRIMIDO (DIRECTIVA VCORE): "Matar el Template". Solo se entregan binarios originales.
        ...
        try:
            ...
            # AGREGAR TODOS LOS PDFS ENCONTRADOS (Directiva Frente 3)
