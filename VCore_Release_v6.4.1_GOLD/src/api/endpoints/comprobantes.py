...
# ==============================================================================
# 🎯 NUEVO ENDPOINT VCORE L6: DESCARGA INTELIGENTE (ZIP o DIRECTO)
# ==============================================================================
@router.get("/{uuid}/download-zip")
async def download_zip_comprobante(
    uuid: str, 
    db: AsyncSession = Depends(get_db),
    entidad_id: uuid.UUID = Depends(get_active_entidad)
):
    """
    ⚡ MOTOR DE DESCARGA INTELIGENTE VCORE: 
    Si existe 1 solo archivo (ej. un PDF huérfano), lo descarga nativamente. 
    Si hay 2 o más (XML + PDFs), arma el ZIP al vuelo.
    """
    res = await db.execute(select(Comprobante).where(func.lower(cast(Comprobante.uuid, String)) == uuid.lower()))
    comp = res.scalars().first()
    
    if not comp or (entidad_id and comp.entidad_id != entidad_id):
    ...
    # 2. Lógica de Producto (UX)
    if len(archivos_validos) == 0:
        raise HTTPException(status_code=404, detail="No hay archivos físicos disponibles para este comprobante.")
        
    elif len(archivos_validos) == 1:
        # Descarga directa sin ZIP (Experiencia de Usuario Optimizada)
        ruta_unica = archivos_validos[0]
        return FileResponse(
            path=ruta_unica, 
            filename=os.path.basename(ruta_unica),
            headers={"Content-Disposition": f"attachment; filename={os.path.basename(ruta_unica)}"}
        )
        
    else:
        # Empaquetado ZIP para 2 o más archivos
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=VCore_{comp.folio or uuid}.zip"}
        )
