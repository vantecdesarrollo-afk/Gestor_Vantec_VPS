...
async def main():
    ...
    print(f"--- FIXING 305 BY UUID: {target_uuid} ---")
    try:
        async with AsyncSessionLocal() as session:
            ...
            # Query by UUID instead of folio
            ...
            if comp:
                ...
                if comp.ruta_resguardo:
                    files = glob.glob(os.path.join(comp.ruta_resguardo, f"*{target_uuid}*.xml"))
                    if files:
                        xml_path = files[0]
                        ...
                        print("❌ XML NOT Found in ruta_resguardo.")
            else:
