...
async def get_comprobantes(
    ...
    try:
        # Security Hardening: get_active_entidad handles entitlement verification
        if not entidad_id:
        ...
        # 2. FILTRADO MANDATORIO L6 (Filtro por entidad validada)
        query = query.where(Comprobante.entidad_id == entidad_id)
        # 2.5 FILTRO UNIVERSAL (Rango de Fechas L6)
        # SSoT: Si existe búsqueda por folio/serie, ignoramos fechas para evitar "falsos negativos"
        if not search:
            if fecha_desde:
                try:
        ...
        # 3. Búsqueda secundaria Normalizada (CURA ATÓMICA L6 v12.0)
        if search:
            search_raw = search.strip()
            # Limpiamos el input: quitamos guiones y espacios
            search_clean = search_raw.replace('-', '').replace(' ', '')
            # Sacamos solo los números para búsqueda numérica (FIX: filter recibe función, no bool)
            search_numeric = "".join(filter(str.isdigit, search_clean)).lstrip('0')

            # Normalización de DB: Folio sin ceros a la izquierda
            ...
            # 7. [v12.0] Búsqueda atómica por Serie + Folio separados (AAC-2496 -> serie=AAC, folio=2496)
        ...
        # --- TOTALIZADOR (Basado en el query ya filtrado) ---
        ...
        if row_uuids:
             ...
             for row_inv in inv_rels_q.all():
                  ...
                  # [BALA DE PLATA VCORE] Búsqueda recursiva física para Asociados (AR-7227 Fix)
                  if not pdf_real_exists:
                       ...
                       try:
                           ...
                           # LA ORDEN DEL CORONEL: Si tienes el XML, el PDF está en la misma carpeta
        ...
        # 4c. Batch lookup for paths of related documents (Comprobante is now SSoT)
        ...
        # --- ELIMINACIÓN DE FALLBACK CONCURRENTE (CAUSA DE FREEZE V-CORE) ---
        # No se permiten búsquedas gloabales (glob) en tiempo de respuesta del API. 
        # La integridad de la ruta debe ser garantizada por el watcher en ingesta.
        async_fallback_map = {}
        ...
        for r in rows:
            try:
                ...
                # --- VERIFICACIÓN DE ARCHIVOS DURA DESDE PERSISTENCIA (SIN SUPOSICIONES LOCALS) ---
                ruta_xml = r.xml_path or r.ruta_resguardo or ""
                ruta_pdf_raw = r.pdf_path or ""
                
                # Soporte para múltiples PDFs (Piped)
                ...
                # --- [L6 v6.0 DYNAMIC RECOVERY - logs/duplicates] ---
                app_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                if not xml_exists:
                    dup_xml = os.path.join(app_root, "Operacion_CFDI", "logs", "duplicates", f"{r.uuid}.xml")
                    if os.path.exists(dup_xml):
                        xml_exists = True
                        ruta_xml = dup_xml
                
                if not pdf_exists:
                    dup_pdf = os.path.join(app_root, "Operacion_CFDI", "logs", "duplicates", f"{r.uuid}.pdf")
                    if os.path.exists(dup_pdf):
                        pdf_exists = True
                        pdf_list = [dup_pdf]
                
                # --- [L6 PROTOCOL: HARD-LINK RECOVERY] ---
                # Si no hay ruta en DB, buscar físicamente en la bóveda por UUID para evitar íconos grises falsos
                ...
                # Lógica de Estatus Inteligente (VCORE-BRIDGE / VCORE L6)
                # PRIORIDAD MÁXIMA: AUSENCIA (Huerfanía en SSoT)
                ...
                # --- [L6 v6.3: TRACE DE RENDERIZADO REMOVED] ---
...
async def export_comprobantes(
    ...
    """
    Sincronización Atómica L6 v34.0: Estatus VCORE 360° y Exportación Dual.
    """
    try:
        from datetime import datetime
        import pandas as pd
        ...
        # 1. Query Base con Relacionados 360° (Directiva CTO)
        query = (
            select(Comprobante)
        ...
        # 2. Batch Calculation de Pagos (Mirroring Dashboard Logic)
        all_uuids = [str(r.uuid).lower() for r in rows]
        
        # 2a. Pagos realizados POR el documento (si es P)
        pagos_sum = {}
        if all_uuids:
             sum_p_batch = await db.execute(
        ...
        # 2b. Pagos recibidos POR las facturas (si son PPD)
        ppd_sum = {}
        if all_uuids:
             sum_ppd_batch = await db.execute(
        ...
        # 2c. Mapa de Relacionados para Trazabilidad
        related_info_map = {}
        if all_uuids:
        ...
        # 3. Construcción de Dataset Unificado
        data_list = []
        for r in rows:
            u_low = str(r.uuid).lower()
            
            # Cálculo de Estatus Sincronizado VCORE
            if r.tipo_comprobante == 'P':
            ...
            # SSSoT: Solo sumar ingresos vigentes para el gran total financiero
        ...
        # 4. Generación de Formatos (Dual Flow)
        if fmt == "csv":
            output = io.StringIO()
        ...
        # XLSX Formatted (Directiva Azul Vantec)
        output = io.BytesIO()
        try:
            if df.empty:
                # Crear Excel vacío con headers si no hay datos
...
async def get_comprobante_xml(uuid: str, db: AsyncSession = Depends(get_db)):
    ...
    if not actual_path or not os.path.exists(actual_path):
        # OMITIDO: El glob recursivo causaba congelamiento de proceso.
        # Fallback local simple si estuviera en la raíz
        p_root = os.path.join("Operacion_CFDI", "Upload_Universal", f"{uuid}.xml")
        if os.path.exists(p_root): actual_path = p_root
        
        # [L6 v6.0] Fallback DynRec a logs/duplicates
        if not actual_path or not os.path.exists(actual_path):
             app_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
             p_dup = os.path.join(app_root, "Operacion_CFDI", "logs", "duplicates", f"{uuid}.xml")
...
async def get_comprobante_pdf(uuid: str, index: int = Query(0), db: AsyncSession = Depends(get_db)):
    ...
    # 1. SINCERAMIENTO BINARIO ATÓMICO (Fix en comprobantes.py) - Directiva v22.0
    if not actual_path:
        from src.services.cfdi_storage import get_cfdi_vault_path
        try:
            vault_dir = ""
            ...
            if os.path.exists(vault_dir):
                # Solo búsqueda DIRECTA y EXACTA por UUID, nada de índices alfabéticos (No más Proformas en Pagos)
                for f in os.listdir(vault_dir):
                    if f.lower() == f"{str(comp.uuid).lower()}.pdf":
                        actual_path = os.path.join(vault_dir, f)
                        # SOBREESCRIBIR DB - AUTO-SINCERAMIENTO
    ...
    # Bloqueo estricto del "Combo" y PDF genéricos
    if not actual_path or not os.path.exists(actual_path):
    ...
    # v12.0: Nombre profesional (Serie + Folio limpio, e.g., AAC2496.pdf)
...
async def get_comprobante_detail(uuid: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # 1. Intentar de CfdiConcepto (SSoT)
        try:
        ...
        # FALLBACK: Extraer del XML
        if not conceptos_output:
            try:
                import xml.etree.ElementTree as ET
                # SSoT: Vault Rescue for Detail Drawer
                from src.services.cfdi_storage import find_cfdi_attachments
                att = find_cfdi_attachments(uuid, comp.serie or "", comp.folio or "", comp.tipo_comprobante or "I")
                actual_path = comp.xml_path if comp and comp.xml_path and os.path.exists(comp.xml_path) else att["xml_path"]
                if actual_path:
