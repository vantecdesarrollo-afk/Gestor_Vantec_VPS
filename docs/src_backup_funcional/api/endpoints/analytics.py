...
async def get_dashboard(
    ...
    try:
        ...
        if not entidad_id:
            # L5 Aislamiento de Datos: No permitir consultas globales "sin filtro" (fuga de datos)
            # Retornar estructura en ceros (o un conteo global neutral)
            import os
            upload_files = 0
            try:
                upload_dir = r"C:\Test_Antigravity\Gestor_CFDI_Vantec\Operacion_CFDI\Upload_Universal"
        ...
        # 1. Conceptos (SELECT DISTINCT ILIKE - Profundidad de Búsqueda v48.0)
        ...
        # Si hay texto de búsqueda, filtrar sugerencias (Marvel Test)
        if concepto and len(concepto) >= 3:
             q_concepts_list = q_concepts_list.where(CfdiConcepto.descripcion.ilike(f"%{concepto}%"))
             
        q_concepts_list = q_concepts_list.limit(20) # Mínimo 20 sugerencias por Directiva CTO
        ...
        # 3. Top Clientes (Contabilizando solo Ingresos netos 'I')
        ...
        # 4. PPD Pendientes (Sincronización L6 con Vista Materializada)
        # REQUERIMIENTO: Usar lógica de Saldo Vivo (saldo_insoluto > 0)
        from sqlalchemy import text
        q_ppd = select(func.count()).select_from(text("v_ppd_semaforo_status"))
        q_ppd = q_ppd.where(text("saldo_insoluto >= 1.00")) # REGLA CTO: Umbral de tolerancia $1.00
        if entidad_id:
        ...
        # 5. Pagos Huérfanos (Vantec Core L6)
...
async def get_sabana_atomica_report(
    ...
    """
    Reporteador Maestro 'Sábana Atómica' (Directiva L6 v5.1).
    Filtro de ADN: Permite exportar versiones 3.3 o 4.0 de forma independiente.
    """
    if not entidad_id:
    ...
    try:
        ...
        output = io.StringIO()
        # UTF-8-SIG para Excel Compatibility
        output.write('\ufeff')
...
async def export_data(
    ...
    """
    Sábana Maestra de Auditoría (Directiva v60.0 - Vantec Senior Fix).
    Recupera las 14 columnas obligatorias y calcula el Estatus VCORE con precisión de $0.01.
    Diferencia entre Streaming CSV y Binario XLSX real.
    """
    try:
        from src.database.models import Comprobante, CfdiConcepto, CfdiRelacionado
        ...
        # 3. Datos de Conciliación y Relaciones (Enriquecido v60.0)
        ...
        # Mapa de UUID a Folio para trazabilidad humana
        ...
        for rel in all_rels:
            ...
            # Enriquecimiento Directiva v60.0: [Folio_Pago] | UUID_Relacionado
        ...
        # 4. Lógica Diferenciada por Formato (Directiva v60.0)
        columns = [
        ...
        if format == 'xlsx':
            # --- EXPORTACIÓN EXCEL REAL (Binary Buffer) ---
            data_list = []
            for r in rows:
            ...
            # --- EXPORTACIÓN CSV STREAMING (Anti-Collapse) ---
