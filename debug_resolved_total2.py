...
async def main():
    ...
    with open("resolved_total_debug2.txt", "w", encoding="utf-8") as f:
        try:
            async with AsyncSessionLocal() as session:
                await session.execute(text("SELECT set_config('app.current_tenant_id', :tid, true)"), {"tid": entidad_id})
                
                query = select(Comprobante).options(selectinload(Comprobante.relaciones)).where(Comprobante.uuid == target_uuid)
                result = await session.execute(query)
                comp = result.scalars().first()

                if comp:
                    f.write("--- CALCULATION DEBUG 2 ---\n")
                    f.write(f"comp.tipo_comprobante: '{comp.tipo_comprobante}'\n")

                    # Use same variables with print execution
                    reps_list = [
                        {
                    ...
                    f.write(f"reps_list items count: {len(reps_list)}\n")
                    if reps_list:
                    ...
                    f.write(f"Condition p_cond: {p_cond}\n")

                    resolved_total = total_monto_pago if p_cond and total_monto_pago > 0 else float(comp.total or 0)
