...
async def patch_database():
    print("🚀 VANTEC SCHEMA PATCH: Iniciando sincronización de columnas...")
    
    # Comandos SQL para renombrar columnas y estandarizar el esquema
    commands = [
        # 1. Estandarización de IDs y Nombres en Usuarios
        "ALTER TABLE usuarios RENAME COLUMN usuario_id TO id;", # Por si acaso
        ...
        "ALTER TABLE entidades_fiscales RENAME COLUMN entidad_id TO id;", # Por si acaso
        ...
        # 4. Limpieza de tablas relacionadas (quitar tenant_id si existe)
    ...
    async with engine.begin() as conn:
        for cmd in commands:
            try:
                ...
                # Si la columna ya fue renombrada, ignoramos el error específico 
                # (PostgreSQL: 42703 undefined_column o 42701 duplicate_column)
